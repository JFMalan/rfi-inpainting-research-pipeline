...
MAX_UNITS=${MAX_UNITS:-}
OUTCOL=${OUTCOL:-ORACLE_PFIX_DATA}
...
echo "STAGE A (build phase-fix preds, ASTRO-PY3.10) node $(hostname)  ms=$MS h5=$H5 -> $PREDS"
singularity exec $ASTROPY python $ROOT/inference/oracle_phasefix.py \
    --ms "$MS" --h5 "$H5" --out-preds "$PREDS" $SIMARG $MU
...
export OUT=${OUT:-/idia/users/$USER/rfi/viz/oracle_pfix}
