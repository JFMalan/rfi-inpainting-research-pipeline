...
TORCH=/idia/software/containers/ASTRO-GPU-PyTorch-2026-01-28.sif
...
echo "--- variant B: scale+mask + correlated speckle ---"
singularity exec $ASTROPY python $SCRIPTS/data_preparation/simulated/realify.py \
    --input $CLEAN --output $WORK/v_speckle.h5 \
    --amp-std 0.21 --speckle-std $SPECKLE --corr-len $CORR \
...
run_test () {
    ...
    singularity exec --nv $TORCH python $SCRIPTS/model/diagnostics/overfit_test.py \
        --data $data --n 24 --iters 800 --bs 8 \
        --predict $pred --eta $eta --hole-fill mean || echo "  [test $tag FAILED]"
}

# baseline: does scale+mask-only still train well under x0? (sanity)
run_test $WORK/v_nospeckle.h5 x0  0.0 "nospeckle-x0"
# speckle under x0 (expected: plateau / smooth — reproduce prior failure mode)
run_test $WORK/v_speckle.h5   x0  0.0 "speckle-x0"
# speckle under eps + stochastic (expected: converges, texture ~1.0)
run_test $WORK/v_speckle.h5   noise 1.0 "speckle-eps-eta1"
# speckle under eps deterministic (point-estimate on smooth part)
