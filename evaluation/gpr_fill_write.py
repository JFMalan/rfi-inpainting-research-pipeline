...
from classical_fill import gpr_fill
...
def main(args):
    ...
    log(f"GPR-fill into {args.out_col}  runs={len(runs)}  n_baseline={n_baseline}  npol={npol}  "
        f"band={N}  ell={args.gpr_ell} noise={args.gpr_noise}")
    ...
    for (tlo, nt), units in runs.items():
        ...
        for p in range(npol):
            ...
            d[..., p] = gpr_fill(V, g, args.gpr_ell, args.gpr_noise).reshape(nt, n_baseline, N).astype(d.dtype)
        ...
        log(f"  run tlo={tlo} nt={nt}: {len(units)} tiles, {int(hole.sum())} hole cells/pol GPR-filled")

    root.flush(); root.close(); hf.close()
    log(f"done: GPR-filled {len(runs)} runs -> {args.out_col}")
...
if __name__ == '__main__':
    ...
    ap.add_argument('--out-col', default='GPR_DATA', dest='out_col')
