...
def main(args):
    ...
    log(f"mean-fill into {args.out_col}  runs={len(runs)}  n_baseline={n_baseline}  npol={npol}  band={N}")
    ...
    for (tlo, nt), units in runs.items():
        ...
        good = ~hole
        cnt = np.maximum(good.sum(axis=0), 1)                       # (n_baseline, N)
        for p in range(npol):
            dp = d[..., p]                                          # view (nt, n_baseline, N)
            meanc = np.where(good, dp, 0).sum(axis=0) / cnt         # (n_baseline, N) per-channel time-mean
            dp[hole] = np.broadcast_to(meanc, (nt, n_baseline, N))[hole]
        ...
        log(f"  run tlo={tlo} nt={nt}: {len(units)} tiles, {int(hole.sum())} hole cells/pol filled")

    root.flush(); root.close(); hf.close()
    log(f"done: mean-filled {len(runs)} runs -> {args.out_col}")
