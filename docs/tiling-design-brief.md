...
pipeline. Read this in full, plus `docs/imaging-investigation-handover.md` and the auto-memory notes
`project_oracle_localization.md` / `project_resolution_research.md` / `project_sim_dataset_inventory.md`,
**before proposing anything**. Present the design (end of this doc) for sign-off **before** writing code.
...
- A CNN inpainter ("U-Paint") FABRICATED fine-frequency structure → 10^4 high-delay power error vs 10^1
  classical. Learned inpainters inventing fine structure is the key danger.
...
- Decision: FREQUENCY TILING with overlap, REUSE the 512 model (NOT a 1024 model, NOT cascaded
  diffusion). Keep the SMOOTH/decompose target (defence vs fabrication). ADD a delay-space metric.
...
5. `evaluation/compare_images.py` — ADD a delay/power-spectrum metric (FFT along frequency of the
   per-baseline visibilities; inpainted vs clean vs flagged). Keep continuum off-source RMS + RMSE-vs-clean.
...
- Run a phase-fix-style oracle on the TILED h5 and image it (reuse `oracle_phasefix.py` + `image_eval.sh`
  + the new delay metric): does the perfect-fill ceiling now beat flagging on BOTH continuum RMSE and
  delay space? Only if yes does the retrain make sense.
