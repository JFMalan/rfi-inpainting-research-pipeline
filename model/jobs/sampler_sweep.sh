...
singularity exec --nv $NVBIND $GPU python $ROOT/model/sampler_sweep.py \
    --data $DATASET --ckpt $CKPT --out $OUT \
    --n $N --predict $PREDICT --steps $STEPS \
    --etas 0.0 0.5 1.0 --repaint-u 1 5

echo "rendering"
singularity exec $GPU python $ROOT/model/viz_eta.py --input $OUT --output $RUNDIR/sweep.png --n $N
