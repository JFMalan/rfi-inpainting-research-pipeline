...
A1=$(env SIM=1 MS=$SIM_MS H5=$SIM_H5 sbatch --parsable tests/jobs/oracle_level0.sh)
echo "  -> $A1"
echo "[A2] phase-fix oracle (cos/sin-resize; isolates the phase-angle-resize suspect). afterok A1 (same MS)"
A2=$(env SIM=1 MS=$SIM_MS H5=$SIM_H5 sbatch --parsable --dependency=afterok:$A1 archive/inference/jobs/oracle_phasefix.sh)
echo "  -> $A2 (waits on $A1)"
